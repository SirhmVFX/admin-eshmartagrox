        ...
        <div>
            ...
            {value ? (
                <div className="relative border border-gray-200 inline-block">
                    <Image src={value} alt="Uploaded" width={320} height={200} className="w-full max-w-xs h-40 object-cover" />
                    <button type="button" onClick={() => onChange("")} className="absolute top-1 right-1 bg-black/60 text-white p-0.5" title="Remove image"><MdClose size={14} /></button>
                </div>
            ) : (
                <div className="border border-dashed border-gray-300 p-8 text-center cursor-pointer hover:border-green-400 transition-colors" onClick={() => inputRef.current?.click()}>
