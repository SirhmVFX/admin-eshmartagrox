...
export default function WysiwygEditor({
    ...
    const ToolBtn = ({
        ...
        <button type="button" onClick={onClick} title={title} className={active ? "is-active" : ""}>
            {children}
        </button>
    );
    ...
    return (
        <div className="tiptap-editor">
            <div className="tiptap-toolbar">
                ...
                <span className="border-l border-gray-300 mx-1 self-stretch" />
                <ToolBtn onClick={() => editor.chain().focus().toggleBulletList().run()} active={editor.isActive("bulletList")} title="Bullet List">UL</ToolBtn>
                <ToolBtn onClick={() => editor.chain().focus().toggleOrderedList().run()} active={editor.isActive("orderedList")} title="Ordered List">OL</ToolBtn>
                <ToolBtn onClick={() => editor.chain().focus().toggleBlockquote().run()} active={editor.isActive("blockquote")} title="Blockquote">{`"`}</ToolBtn>
                <span className="border-l border-gray-300 mx-1 self-stretch" />
                <ToolBtn onClick={setLink} active={editor.isActive("link")} title="Add Link">Link</ToolBtn>
                <ToolBtn onClick={() => fileInputRef.current?.click()} title="Insert Image" active={uploading}>
                    {uploading ? "…" : "Img"}
                </ToolBtn>
                <span className="border-l border-gray-300 mx-1 self-stretch" />
